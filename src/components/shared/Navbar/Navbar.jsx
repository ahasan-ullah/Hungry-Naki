      ...
      user?.email ? (
        <div className="flex items-stretch space-x-3">
          <div className="dropdown dropdown-end">
            <div tabIndex={0} role="button" className="border border-blue-500 rounded-full">
              <img className="w-12" src={user.photoURL} alt="user image" />
            </div>
            <ul
              tabIndex={0}
              className="menu dropdown-content bg-base-100 rounded-box z-[1] mt-4 w-52 p-2 shadow">
              <li><NavLink to={`/myFood/${user.email}`}>My Foods</NavLink></li>
              <li><NavLink to={'/addFood'}>Add Food</NavLink></li>
              <li><NavLink to={`/myOrders/${user.email}`}>My Orders</NavLink></li>
            </ul>
          </div>
          <button onClick={handleLogout} className="btn btn-error text-white">Logout</button>
        </div>
      ) : (
        <NavLink to={'/login'} className="btn btn-accent text-white">Login</NavLink>
      )
    }
  </div>
</div>
